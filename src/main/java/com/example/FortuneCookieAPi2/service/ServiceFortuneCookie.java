package com.example.FortuneCookieAPi2.service;
...
@Service
public class ServiceFortuneCookie {
    //random to retrive random cookie
    private final Random random = new SecureRandom();

    //getCookie method (for 1 cookie)
    public String getCookie(){
        return getCookies(1).get(0);
    }
    //list of cookies (for x number of cookies)
    public List<String> getCookies(int number){
        assert(number >0);
        List<String> cookies = new LinkedList<>();
        for(int i = 0; i < number; i++){
            int randCookies = random.nextInt(COOKIES.length);
            cookies.add(COOKIES[randCookies]);

        }
        return cookies;
    }
    
    //final array for cookies 
